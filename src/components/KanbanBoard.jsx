              ...
              {getClientsByStatus(status).map(client => {
                ...
                return (
                  <div key={client.id} className="client-card" draggable onDragStart={(e) => handleDragStart(e, client.id)} onClick={() => setActiveClient(client)}>
                    ...
                    <p style={{ margin: '2px 0', color: c('#2d3748','#e2e8f0') }}>📞 {client.phone}</p>
                    {client.address && (
                    ...
                    <p style={{ color: c('#4a5568','#94a3b8') }}>💰 Budżet: <strong>{Number(client.budget).toFixed(2)} PLN</strong></p>
                    ...
                    {editor && (
                      <p style={{ color: c('#a0aec0', '#64748b'), fontSize: '11px', marginTop: '6px', borderTop: `1px dashed ${c('#e2e8f0','#334155')}`, paddingTop: '4px' }}>
